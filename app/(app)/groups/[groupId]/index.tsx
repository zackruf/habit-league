...
export default function GroupScreen() {
  ...
  const { getGroupDetails, getGroupMessages, session } = useApp();
  ...
  const latestMessage = useMemo(
    () => [...messages].sort((left, right) => right.createdAt.localeCompare(left.createdAt))[0],
    [messages]
  );
      ...
      {activeTab === 'overview' ? (
        <>
          <GroupSummaryCard
            ...
            latestChatPreview={latestMessage ? `${latestMessage.senderName}: ${latestMessage.text}` : null}
            onEdit={isOwner ? () => router.push(`/(app)/groups/${details.group.id}/edit`) : undefined}
          />
