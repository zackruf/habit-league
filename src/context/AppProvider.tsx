...
import {
  ...
  setHabitProof as setHabitProofRequest,
...
type AppContextValue = {
  ...
  setHabitProof: (habitId: string, dateKey: string, proofUri: string) => Promise<void>;
...
export function AppProvider({ children, fallback }: PropsWithChildren<{ fallback?: ReactNode }>) {
  ...
  const setHabitProof = useCallback(
    async (habitId: string, dateKey: string, proofUri: string) => {
      if (!session) {
        return;
      }

      await setHabitProofRequest(session.uid, habitId, dateKey, proofUri);
      await refreshUserData(session);
    },
    [refreshUserData, session]
  );
  ...
  const value = useMemo<AppContextValue>(
    ...
    [authReady, busy, refreshing, session, profile, habits, groups, signIn, signUp, signOut, saveProfile, createHabit, toggleHabitCheckIn, setHabitProof, createGroup, joinGroup]
  );
