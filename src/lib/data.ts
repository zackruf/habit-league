...
export async function toggleHabitCheckIn(uid: string, habitId: string) {
  ...
  if (usingFirebaseBackend && firestore) {
    ...
    const isChecked = habit.checkIns.includes(todayKey);
    await updateDoc(habitRef, {
      checkIns: isChecked ? arrayRemove(todayKey) : arrayUnion(todayKey),
      ...(isChecked ? { [`proofs.${todayKey}`]: deleteField() } : {}),
    });
    return;
  }
  ...
  const isChecked = habit.checkIns.includes(todayKey);
  const nextCheckIns = isChecked
    ? habit.checkIns.filter((entry) => entry !== todayKey)
    : [...habit.checkIns, todayKey];
  const currentProofs = habit.proofs ?? {};
  const nextProofs = isChecked
    ? Object.fromEntries(Object.entries(currentProofs).filter(([key]) => key !== todayKey))
    : currentProofs;

  store.habits[habitId] = { ...habit, checkIns: nextCheckIns, proofs: nextProofs };
  await writeDemoStore(store);
}

export async function setHabitProof(uid: string, habitId: string, dateKey: string, proofUri: string) {
  if (usingFirebaseBackend && firestore) {
    const habitRef = doc(firestore, 'habits', habitId);
    const snapshot = await getDoc(habitRef);
    if (!snapshot.exists()) {
  ...
  store.habits[habitId] = {
    ...habit,
    proofs: { ...(habit.proofs ?? {}), [dateKey]: proofUri },
  };
  await writeDemoStore(store);
}
